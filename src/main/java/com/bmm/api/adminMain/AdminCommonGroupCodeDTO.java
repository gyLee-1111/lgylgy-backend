package com.bmm.api.adminMain;

import java.time.LocalDate;

public class AdminCommonGroupCodeDTO {
	
	private String commonGroupCode;
	
	private String commonGroupNm;
	
	private String description;
	
	private String useYn;
	
	private String userId;
	
	private LocalDate insertDt;

	public String getCommonGroupCode() {
		return commonGroupCode;
	}

	public void setCommonGroupCode(String commonGroupCode) {
		this.commonGroupCode = commonGroupCode;
	}

	
	public String getCommonGroupNm() {
		return commonGroupNm;
	}

	public void setCommonGroupNm(String commonGroupNm) {
		this.commonGroupNm = commonGroupNm;
	}

	public String getDescription() {
		return description;
	}

	public void setDescription(String description) {
		this.description = description;
	}

	public String getUseYn() {
		return useYn;
	}

	public void setUseYn(String useYn) {
		this.useYn = useYn;
	}

	public String getUserId() {
		return userId;
	}

	public void setUserId(String userId) {
		this.userId = userId;
	}

	public LocalDate getInsertDt() {
		return insertDt;
	}

	public void setInsertDt(LocalDate insertDt) {
		this.insertDt = insertDt;
	}



}
